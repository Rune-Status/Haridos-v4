...
public class CacheFile {
    ...
    public CacheFile(byte[] bytes) throws IOException {
        data = ByteBuffer.wrap(bytes);
        compression = bytes[0];
        length = BufferUtils.readInt(1, bytes);
        if (compression != 0) {
            this.length = BufferUtils.readInt(5, bytes);
        }
    }

    public CacheFile(int container, int fileId, int compression, int length,
	    ByteBuffer data) {
    ...
    public byte[] decompress(int[] key) throws IOException {
	data.rewind();
	int compression = data.get() & 0xff;
	int length = data.getInt();
	int decompressedLength = compression != COMPRESSION_NONE ? data
		.getInt() : length;
	...
	return getBytes(!decompressed ? (compression != COMPRESSION_NONE ? 9
		: 5) : 0);
    }
	...
	int subFileCount = Cache.INSTANCE.getContainer(container)
		.getFileDescriptor().getEntries()[fileId].getSubFiles().length;
	if (subFileCount == 0) {
	    throw new UnsupportedOperationException(
		    "This file does not contain any sub files!");
	}
	...
	for (int i_27_ = 0; i_27_ < i_19_; i_27_++) {
		...
		i_28_ += data.getInt();
		System.arraycopy(dataArray, readPos, subData[subId],
