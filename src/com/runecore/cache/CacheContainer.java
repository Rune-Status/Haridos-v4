...
/**
 * This distributes all cache data. Named 'filestore' in Method's cache
 * downloader
 * 
 * @author 'Mystic Flow
 * @author `Discardedx2
...
public class CacheContainer {
     ...
     *            The RandomAccessFile we use for reading data off the cache
     */
    public CacheContainer(int container, RandomAccessFile mainFileStore,
	...
	try {
	    ByteBuffer fileBuffer = containerFile.getChannel().map(
		    FileChannel.MapMode.READ_ONLY, file * 6, 6);
	    int length = ((fileBuffer.get() & 0xFF) << 16)
		    | ((fileBuffer.get() & 0xFF) << 8)
		    | (fileBuffer.get() & 0xFF);
	    int position = ((fileBuffer.get() & 0xFF) << 16)
		    | ((fileBuffer.get() & 0xFF) << 8)
		    | (fileBuffer.get() & 0xFF);
	    if (length == 0) {
		...
		ByteBuffer mainBuffer = mainFileStore.getChannel().map(
		...
		position = ((mainBuffer.get() & 0xFF) << 16)
	    ...
	    return new CacheFile(container, file, compression, fileLength,
		    buffer);
	} catch (Exception e) {
	    return null;
	}
	...
	RS2FileDescriptor entry = fileDescriptor.getDescriptorMap().get(
		RS2FileDescriptorTable.hashName(fileName));
	if (entry == null) {
	    return null;
	}
