...
public class CacheItemDefinition {
	...
	public static void loadEquipIds() {
		...
		for (int i = 0; i < ItemDefinition.MAX_SIZE; i++) {
			...
			CacheItemDefinition cacheDef = def.getCacheDefinition();
			if (cacheDef.maleWornModelId1 >= 0 || cacheDef.maleWornModelId2 >= 0) {
				def.setEquipId(equipId++);
			}
			def.setName(cacheDef.name);
		}
		*/
	}
		...
		for (int opcode2 : clientScriptData.keySet()) {
			...
			if (opcode2 >= 749 && opcode2 < 797) {
				...
				if (levelId != -1 && levelReq != -1) {
					...
					if (id == 2503) {
						levelRequirements.put(4, 40);
					} else if (name.toLowerCase().contains("infinity")) {
