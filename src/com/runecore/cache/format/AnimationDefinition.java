...
public class AnimationDefinition
{
	...
	public static AnimationDefinition forId(int animId) {
		AnimationDefinition def = ANIMATIONS.get(animId);
		if (def != null) {
			return def;
		}
		try {
			byte[] data = CacheManager.getData(CacheConstants.ANIM_IDX_ID, animId >>> 7, animId & 0x7f);
	...
	private final void method540(int i, int i_0_, ByteBuffer buffer) {
		if (i_0_ == 1) {
			...
			if (i_0_ == 12) {
				int i_8_ = buffer.get() & 0xFF;
				anIntArray615 = new int[i_8_];
				for (int i_9_ = 0; i_9_ < i_8_; i_9_++)
					anIntArray615[i_9_] = buffer.getShort() & 0xFFFF;
				for (int i_10_ = 0; i_8_ > i_10_; i_10_++)
					anIntArray615[i_10_] = (buffer.getShort() & 0xFFFF << 16) + anIntArray615[i_10_];
			} else if (i_0_ == 13) {
				int i_11_ = buffer.getShort() & 0xFFFF;
				anIntArrayArray613 = new int[i_11_][];
				for (int i_12_ = 0; i_12_ < i_11_; i_12_++) {
					int i_13_ = buffer.get() & 0xFF;
					if (i_13_ > 0) {
						anIntArrayArray613[i_12_] = new int[i_13_];
						anIntArrayArray613[i_12_][0] = BufferUtils.getMediumInt(buffer);//buffer.getMedium((byte) 55); //TODO
						for (int i_14_ = 1; i_13_ > i_14_; i_14_++)
							anIntArrayArray613[i_12_][i_14_] = buffer.getShort() & 0xFFFF;
					}
				}
			} else if (i_0_ != 14) {
				if (i_0_ == 15) {
	...
	final void method541(ByteBuffer buffer) {
		for (;;) {
			int i_19_ = buffer.get() & 0xFF;
			if (i_19_ == 0) {
				break;
			}
	...
	public int getDuration() {
		int total = 0;
		for (int delay : frameDelay) {// = 0; i < frameDelay.length; i++) {
			total += delay;
		}
